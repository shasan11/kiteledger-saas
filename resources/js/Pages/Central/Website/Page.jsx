...
import { FeatureIcon, PageHeader, WebsiteButton, WebsiteContainer } from './components/WebsitePrimitives';
...
function WebsiteFeatures({features=[]}){
    ...
    const icons=['chart','invoice','box','people','workflow','shield','spark','clock','layers','bolt'];
    return <section className="kl-features-page"><WebsiteContainer>
        <div className="kl-feature-simple-grid">{features.map((feature,index)=><article key={feature.id||feature.slug}>
            <div className="kl-feature-simple-grid__meta"><FeatureIcon name={icons[index%icons.length]}/><span>{String(index+1).padStart(2,'0')}</span></div>
            <h2>{feature.title}</h2>
            {feature.excerpt&&<p>{feature.excerpt}</p>}
        </article>)}</div>
        <div className="kl-feature-simple-cta"><div><p className="kl-eyebrow">One connected platform</p><h2>Use what your business needs today.</h2><p>Add more workflows as you grow, without rebuilding your data.</p></div><div><WebsiteButton href="/pricing">Start free</WebsiteButton><WebsiteButton href="/contact" variant="secondary">Talk to our team</WebsiteButton></div></div>
