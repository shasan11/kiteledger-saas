...
export default function Sections({ pages, selectedPage, sections }) {
    ...
    const save = (values) => {
        ...
        try {
            ...
            form.setFields([
                {
                    name: "items_json",
                    errors: ["Items and settings must contain valid JSON."],
                },
            ]);
            return;
        }
        ...
        record
            ? router.put(
                  route("central.website-sections.update", record.id),
                  payload,
                  options,
              )
            : router.post(
                  route("central.website-sections.store"),
                  payload,
                  options,
              );
    };
    ...
    const remove = (section) =>
        Modal.confirm({
            ...
            content: "The section will stop rendering on the public page.",
            okText: "Delete section",
            okButtonProps: { danger: true },
            onOk: () =>
                router.delete(
                    route("central.website-sections.destroy", section.id),
                ),
        });
    return (
        <CentralLayout title="Homepage Sections">
            <PageHeader
                eyebrow="Website"
                title="Section Builder"
                description="Compose page sections, repeatable items, calls to action, media, alignment, and reliable ordering."
                actions={
                    <Button
                        type="primary"
                        icon={<PlusOutlined />}
            ...
            <SectionCard>
                <div className="central-toolbar">
                    <Select
                        ...
                        onChange={(page_id) =>
                            router.get(
                                route("central.website-sections.index"),
                                { page_id },
                            )
                        }
                    />
                ...
                <Space direction="vertical" style={{ width: "100%" }} size={10}>
                    {sections.map((section, index) => (
                        <div className="central-section-row" key={section.id}>
                            <span className="central-section-row__order">
                                <Button
                            ...
                            <span className="central-section-row__copy">
                                <Typography.Text strong>
                                    {section.title || section.section_key}
                                </Typography.Text>
                                <span>
                                    <Tag>{section.section_type}</Tag>
                                    <Tag>{section.section_key}</Tag>
                                    {!section.is_active && (
                ...
                {!sections.length && (
                    <div className="central-empty">
                        No sections yet. Add the first section for this page.
                    </div>
                )}
            </SectionCard>
            <Drawer
            ...
            >
                <Form form={form} layout="vertical" onFinish={save}>
                    ...
                    <Form.Item name="content" label="Rich content">
                        <RichTextEditor
                            autosaveKey={`section.${record?.id || "new"}`}
                        />
                    </Form.Item>
                    <div className="central-two-column">
                    ...
                    <div className="central-two-column">
                        ...
                        <Form.Item name="button_url" label="Primary URL">
                            <Input />
                        </Form.Item>
                    </div>
                    <div className="central-two-column">
                    ...
                    <div className="central-two-column">
                        ...
                        >
                            <Input placeholder="#f8fafc or theme token" />
                        </Form.Item>
                        <Form.Item
                            name="alignment"
                            label="Alignment"
                    ...
                    <Form.Item
                        ...
                        rules={[{ validator: jsonValidator }]}
                    >
                        <Input.TextArea rows={5} className="central-code" />
                    </Form.Item>
                    <Space size="large">
