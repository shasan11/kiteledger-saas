...
export default function AdminUsers({ admins, roles }) {
    ...
    const remove = (admin) => { deleteForm.resetFields(); setDeleteTarget(admin); };
    const confirmRemove = (values) => router.delete(route('central.central-admins.destroy', deleteTarget.id), { data: values, preserveScroll: true, onSuccess: () => setDeleteTarget(null) });
    const columns = [
    ...
    return <CentralLayout title="Admin Users">
        ...
        <Drawer open={open} onClose={() => setOpen(false)} width={520} title={record ? 'Edit administrator' : 'Add administrator'} extra={<Button type="primary" onClick={() => form.submit()}>Save</Button>}><Form form={form} layout="vertical" onFinish={save}><Form.Item name="name" label="Name" rules={[{ required: true }]}><Input/></Form.Item><Form.Item name="email" label="Email" rules={[{ required: true, type: 'email' }]}><Input type="email"/></Form.Item><Form.Item name="role_id" label="Role" rules={[{ required: true }]}><Select options={roles.map((role) => ({ value: role.id, label: role.label }))}/></Form.Item><Form.Item name="password" label={record ? 'New password' : 'Password'} extra={record ? 'Leave blank to retain the current password.' : 'Use at least 12 characters.'} rules={record ? [] : [{ required: true }, { min: 12 }]}><Input.Password autoComplete="new-password"/></Form.Item><Form.Item name="current_password" label="Your current administrator password" extra="Required for privileged administrator changes." rules={[{required:true}]}><Input.Password autoComplete="current-password"/></Form.Item><Form.Item name="is_active" label="Active" valuePropName="checked"><Switch/></Form.Item></Form></Drawer>
        <Modal open={Boolean(deleteTarget)} title={`Remove ${deleteTarget?.name || 'administrator'}?`} okText="Remove administrator" okButtonProps={{ danger: true }} onCancel={() => setDeleteTarget(null)} onOk={() => deleteForm.submit()} destroyOnHidden><p>The administrator will immediately lose control-center access. The reason and actor are recorded in the audit log.</p><Form form={deleteForm} layout="vertical" onFinish={confirmRemove}><Form.Item name="reason" label="Reason" rules={[{ required: true }, { min: 10 }]}><Input.TextArea rows={3}/></Form.Item><Form.Item name="current_password" label="Your current administrator password" rules={[{ required: true }]}><Input.Password autoComplete="current-password"/></Form.Item></Form></Modal>
    </CentralLayout>;
}
