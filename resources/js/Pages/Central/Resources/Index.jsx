...
export default function Index({ resource, rows, columns = [], editable = false, fields = [], summary = null, filters = {}, customers = [], auditAdmins = [], auditActions = [] }) {
    ...
    return <CentralLayout title={title}>
        ...
        <SectionCard>
            <div className="central-toolbar"><Input.Search className="central-toolbar__search" value={search} onChange={(event)=>setSearch(event.target.value)} onSearch={()=>apply({page:undefined})} allowClear prefix={<SearchOutlined/>} placeholder={`Search ${title.toLowerCase()}`}/>{customers.length>0&&<Select allowClear showSearch optionFilterProp="label" value={filters.customer_id||undefined} placeholder="All customers" style={{minWidth:220}} options={customers.map(row=>({value:row.id,label:row.company_name}))} onChange={customer_id=>apply({customer_id,page:undefined})}/>} {resource === 'audit-logs' && <><Select allowClear showSearch optionFilterProp="label" value={filters.admin_id ? Number(filters.admin_id) : undefined} placeholder="All administrators" style={{minWidth:190}} options={auditAdmins.map(row=>({value:row.id,label:row.name}))} onChange={admin_id=>apply({admin_id,page:undefined})}/><Select allowClear showSearch value={filters.action || undefined} placeholder="All activities" style={{minWidth:220}} options={auditActions.map(value=>({value,label:humanize(value)}))} onChange={action=>apply({action,page:undefined})}/><Input type="date" aria-label="Audit from date" value={filters.date_from || ''} onChange={event=>apply({date_from:event.target.value || undefined,page:undefined})}/><Input type="date" aria-label="Audit to date" value={filters.date_to || ''} onChange={event=>apply({date_to:event.target.value || undefined,page:undefined})}/></>} {columns.includes('status') && <Select allowClear value={filters.status || undefined} placeholder="All statuses" style={{minWidth:160}} options={['active','pending','success','failed','paid','issued','draft','published','available','allocated'].map((value)=>({value,label:humanize(value)}))} onChange={(status)=>apply({status,page:undefined})}/>}<span className="central-filter-summary">{rows.total || 0} records</span></div>
