...
export default function Show({ tenant }) {
    ...
    return <CentralLayout title={tenant.company_name} breadcrumbs={[{title:'Tenants'}]}>
        ...
        <Modal open={retryOpen} title="Retry tenant provisioning" okText="Run provisioning" onCancel={()=>setRetryOpen(false)} onOk={()=>retryForm.submit()}><Alert type="info" showIcon message="Set the tenant owner's login password" description="The password is required to recover provisioning attempts created before the owner account was completed." style={{marginBottom:16}}/><Form form={retryForm} layout="vertical" onFinish={(values)=>{setRetryOpen(false);post('central.tenants.retry',values);retryForm.resetFields();}}><Form.Item name="owner_password" label="Owner password" rules={[{required:true,min:12,message:'Enter at least 12 characters.'}]}><Input.Password autoComplete="new-password"/></Form.Item><Form.Item name="owner_password_confirmation" label="Confirm owner password" dependencies={['owner_password']} rules={[{required:true,message:'Confirm the owner password.'},({getFieldValue})=>({validator(_,value){return !value || getFieldValue('owner_password')===value ? Promise.resolve() : Promise.reject(new Error('The passwords do not match.'));}})]}><Input.Password autoComplete="new-password"/></Form.Item></Form></Modal>
        ...
        <DeleteCustomerModal open={deleteOpen} onClose={()=>setDeleteOpen(false)} tenant={tenant} verifiedBackups={tenant.backup_manifests || []} />
    </CentralLayout>;
}
