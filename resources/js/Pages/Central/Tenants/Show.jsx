...
export default function Show({ tenant }) {
    ...
    const domains = <SectionCard title="Domains" description="Domains currently associated with this customer"><Table className="central-table" pagination={false} rowKey="id" dataSource={tenant.domains || []} columns={[{title:'Domain',dataIndex:'domain'},{title:'Type',dataIndex:'type'},{title:'Status',render:(_,row)=><StatusBadge value={row.status}/>},{title:'Primary',render:(_,row)=>row.is_primary?'Primary':'—'},{title:'Verified',render:(_,row)=>formatDate(row.verified_at)}]} scroll={{x:650}} /></SectionCard>;
