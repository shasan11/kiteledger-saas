...
const calculateTotals = (values = {}) => {
    ...
    const quoteDiscountValue = toNumber(values.quote_discount);
    const quoteDiscount =
        values.quote_discount_type === 'amount'
            ? quoteDiscountValue
            : (itemTotals.itemTotal * quoteDiscountValue) / 100;

    return {
        ...itemTotals,
        quoteDiscount,
        grandTotal: Math.max(itemTotals.itemTotal - quoteDiscount, 0),
    };
};
...
const initialValues = {
    ...
    terms_and_conditions:
        ...
        '3. Delivery timeline depends on stock availability.\n' +
        '4. Payment shall be made as per agreed credit terms.\n' +
        '5. Any change in scope or quantity may affect pricing.',
...
export default function Index() {
                        ...
                        >
                            ...
                            <div style={rowStyle}>
                                ...
                                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                    <InputNumber
                                        value={values.quote_discount}
                                        min={0}
                                        style={{ width: 140 }}
                                        onChange={(val) => setFieldValue('quote_discount', val || 0)}
                                    />
                                    <Select
                                        value={values.quote_discount_type}
                                        options={discountTypeOptions}
                                        style={{ width: 95 }}
                                        onChange={(val) => setFieldValue('quote_discount_type', val)}
                                    />
                                </div>
                            </div>
        ...
        () => [
            ...
            },
            {
                ...
                render: (value) => {
                    ...
                    if (value === 'sent') return <Tag color="blue">Sent</Tag>;
                    if (value === 'accepted') return <Tag color="green">Accepted</Tag>;
                    if (value === 'rejected') return <Tag color="red">Rejected</Tag>;
                    if (value === 'expired') return <Tag color="orange">Expired</Tag>;
    ...
    const transformPayload = (values) => {
        ...
        return {
            ...
            expiry_date: dayjs(values.expiry_date, 'DD-MM-YYYY').isValid()
                ? dayjs(values.expiry_date, 'DD-MM-YYYY').format('YYYY-MM-DD')
                : values.expiry_date,
            ...
            quote_discount: toNumber(values.quote_discount),
            quote_discount_type: values.quote_discount_type || 'percent',

            sub_total: Number(totals.subTotal.toFixed(2)),
            discount_amount: Number(totals.quoteDiscount.toFixed(2)),
