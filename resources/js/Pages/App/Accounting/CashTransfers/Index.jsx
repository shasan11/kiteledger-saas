...
export default function CashTransfersIndex(props) {
    ...
    const columns = useMemo(() => [
        ...
        { title: 'Total Amount', dataIndex: 'total_amount', key: 'total_amount', sorter: true, align: 'right', width: 150, render: (v, record) => renderAmountWithDefaultCurrency(v, record) },
    ], []);
    ...
    const rowMenu = useMemo(() => [
        {
            ...
            onClick: async ({ selectedRowKeys, fetchData, clearSelection, message }) => {
                try {
                    await axios.patch(api('/api/cash-transfers/bulk'), { records: selectedRowKeys.map((id) => ({ id, approved: true })) });
    ...
    const handleVoidConfirm = async () => {
        ...
        try {
            await axios.patch(api('/api/cash-transfers/bulk'), { records: ctx.selectedRowKeys.map((id) => ({ id, void: true, voided_reason: reason })) });
