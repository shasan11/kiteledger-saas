...
const CANONICAL_SERIALIZER_KEYS = {
    ...
    lines: 'invoice_lines',
    lineKeys: ['invoice_id', 'allocated_amount'],
};

const emptyItem = { invoice_id: null, invoice_id_detail: null, invoice_label: '', allocated_amount: 0 };
...
const validationSchema = Yup.object().shape({
    ...
    items: Yup.array().of(
        Yup.object().shape({
            invoice_id:       Yup.string().nullable().required('Invoice is required'),
...
export default function CustomerPayments() {
    const columns = useMemo(() => [
        {
            ...
            width: 150,
            sorter: true,
            render: v => <Text strong style={{ color: '#1677ff' }}>{v || 'DRAFT'}</Text>,
        },
        {
            title: 'Date',
    ...
    const fields = useMemo(() => [
        ...
        },
        {
            ...
            headerBg: '#1d2939',
            headerColor: '#ffffff',
            addButtonLabel: '+ Add Invoice Line',
            ...
            columns: [
                {
                    ...
                    labelField: 'invoice_label',
                    fkLabel: r => {
                        const no = r?.invoice_no || '';
                        const date = r?.invoice_date ? dayjs(r.invoice_date).format('DD-MM-YYYY') : '';
    ...
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold">Customer Payments</h2>}>
