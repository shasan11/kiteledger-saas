...
export default function CreditNotesIndex(props) {
    ...
    const columns = useMemo(() => [
        ...
        { title: 'Date', dataIndex: 'sales_return_date', key: 'sales_return_date', sorter: true, width: 120, render: displayDate, backendFilter: { type: 'date_range', fromParam: 'date_from', toParam: 'date_to' } },
    ...
    const rowMenu = useMemo(() => [
        {
            ...
            onClick: async ({ selectedRowKeys, fetchData, clearSelection, message }) => {
                try {
                    await axios.patch(api('/api/credit-notes/bulk'), { records: selectedRowKeys.map((id) => ({ id, approved: true })) });
    ...
    const handleVoidConfirm = async () => {
        ...
        try {
            await axios.patch(api('/api/credit-notes/bulk'), { records: ctx.selectedRowKeys.map((id) => ({ id, void: true, voided_reason: reason })) });
    ...
    return (
        <AuthenticatedLayout user={props.auth?.user}>
            ...
            <ReusableCrud
                ...
                rowMenu={rowMenu}
                custom_add={true}
                custom_add_link={route('payment-in.credit-notes.add')}
                form_ui="drawer"
                ...
                activeTableRowFunction={(record) => ({
                    onClick: (event) => {
                        ...
                        router.visit(route('payment-in.credit-notes.show', record.id));
                    },
