...
export default function SalesOrderIndex(props) {
    ...
    const columns = useMemo(() => [
        ...
        { title: 'Date', dataIndex: 'sales_order_date', key: 'sales_order_date', sorter: true, width: 120, render: displayDate },
    ...
    const rowMenu = useMemo(() => [
        {
            ...
            onClick: async ({ selectedRowKeys, fetchData, clearSelection, message }) => {
                try {
                    await axios.patch(api('/api/sales-orders/bulk'), { records: selectedRowKeys.map((id) => ({ id, approved: true })) });
    ...
    const handleVoidConfirm = async () => {
        ...
        try {
            await axios.patch(api('/api/sales-orders/bulk'), { records: ctx.selectedRowKeys.map((id) => ({ id, void: true, voided_reason: reason })) });
