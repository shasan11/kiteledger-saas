...
const statusColor = (s) => ({ draft: 'default', posted: 'blue', paid: 'green', cancelled: 'red' }[s] || 'default');
...
export default function ExpensesIndex(props) {
    ...
    const columns = useMemo(() => [
        ...
        { title: 'Status', dataIndex: 'status', key: 'status', width: 120, render: (v) => <Tag color={statusColor(v)} style={{ textTransform: 'capitalize' }}>{v || 'draft'}</Tag>, backendFilter: { type: 'select', paramName: 'status', options: [{ value: 'draft', label: 'Draft' }, { value: 'posted', label: 'Posted' }, { value: 'void', label: 'Void' }] } },
    ...
    const rowMenu = useMemo(() => [
        {
            ...
            onClick: async ({ selectedRowKeys, fetchData, clearSelection, message }) => {
                try {
                    await axios.patch(api('/api/expenses/bulk'), { records: selectedRowKeys.map((id) => ({ id, approved: true })) });
    ...
    const handleVoidConfirm = async () => {
        ...
        try {
            await axios.patch(api('/api/expenses/bulk'), { records: ctx.selectedRowKeys.map((id) => ({ id, void: true, voided_reason: reason })) });
    ...
    return (
        <AuthenticatedLayout user={props.auth?.user}>
            ...
            <ReusableCrud
                ...
                rowMenu={rowMenu}
                custom_add={true}
                custom_add_link={route('payment-out.expenses.add')}
                form_ui="drawer"
