...
export default function PurchaseBillsIndex(props) {
    ...
    const columns = useMemo(() => [
        ...
        { title: 'Date', dataIndex: 'bill_date', key: 'bill_date', sorter: true, width: 120, render: displayDate },
    ...
    const rowMenu = useMemo(() => [
        {
            ...
            onClick: async ({ selectedRowKeys, fetchData, clearSelection, message }) => {
                try {
                    await axios.patch(api('/api/purchase-bills/bulk'), { records: selectedRowKeys.map((id) => ({ id, approved: true })) });
    ...
    const handleVoidConfirm = async () => {
        ...
        try {
            await axios.patch(api('/api/purchase-bills/bulk'), { records: ctx.selectedRowKeys.map((id) => ({ id, void: true, voided_reason: reason })) });
