...
const navigation = [
    ...
    menu('Customers', <TeamOutlined/>, [entry('central.tenants.index','All Customers',<TeamOutlined/>,'tenant.view'),entry('central.subscriptions.index','Subscriptions',<CreditCardOutlined/>,'subscription.view'),entry('central.usage.index','Usage',<DashboardOutlined/>,'tenant.view'),entry('central.tenant-feature-overrides.index','Feature Overrides',<SettingOutlined/>,'feature_override.manage')]),
    menu('Billing', <BankOutlined/>, [entry('central.dashboard','Revenue Overview',<BankOutlined/>,'dashboard.view'),entry('central.invoices.index','Invoices',<FileTextOutlined/>,'invoice.view'),entry('central.payments.index','Payments',<BankOutlined/>,'payment.view'),entry('central.gateways.index','Payment Gateways',<CreditCardOutlined/>,'gateway.view'),entry('central.invoice-customization.index','Invoice Customization',<FileTextOutlined/>,'invoice.customize')]),
    menu('Support', <MessageOutlined/>, [entry('central.support.tickets.index','Tickets',<MessageOutlined/>,'ticket.view'),entry('central.support-categories.index','Support Categories',<TagsOutlined/>,'support.manage'),entry('central.saved-replies.index','Saved Replies',<MessageOutlined/>,'support.manage'),entry('central.settings.index','Support Settings',<SettingOutlined/>,'settings.view',{group:'support'})]),
    menu('Infrastructure', <DatabaseOutlined/>, [entry('central.tenant-databases.index','Customer Databases',<DatabaseOutlined/>,'system_health.view'),entry('central.provisioning-logs.index','Provisioning Logs',<CloudServerOutlined/>,'system_health.view'),entry('central.backups.index','Backups',<CodeSandboxOutlined/>,'system_health.view'),entry('central.dashboard','System Health',<HeartOutlined/>,'system_health.view')]),
    menu('Products', <AppstoreOutlined/>, [entry('central.plans.index','Plans',<AppstoreOutlined/>,'plan.view'),entry('central.features.index','Features',<AppstoreOutlined/>,'feature.view'),entry('central.default-templates.index','Default Templates',<CodeSandboxOutlined/>,'settings.view')]),
    menu('Website', <GlobalOutlined/>, [
        entry('central.website.overview','Website Overview',<HomeOutlined/>,'cms.view'),
        entry('central.website-pages.index','Pages',<FileTextOutlined/>,'cms.view'),
        entry('central.website-sections.index','Sections',<AppstoreOutlined/>,'cms.manage'),
        entry('central.website-menus.index','Navigation Menus',<GlobalOutlined/>,'cms.manage'),
        menu('Content', <BookOutlined/>, [
            entry('central.blog.index','Blog Posts',<BookOutlined/>,'blog.view'),
            entry('central.blog-categories.index','Blog Categories',<TagsOutlined/>,'blog.manage'),
            entry('central.blog-tags.index','Blog Tags',<TagsOutlined/>,'blog.manage'),
            entry('central.website-faqs.index','FAQs',<QuestionCircleOutlined/>,'cms.manage'),
            entry('central.website-testimonials.index','Testimonials',<TeamOutlined/>,'cms.manage'),
            entry('central.seo.index','SEO Settings',<SearchOutlined/>,'seo.manage'),
            entry('central.settings.index','Website Branding',<ShopOutlined/>,'website.branding.manage',{group:'branding'}),
            entry('central.media.index','Media Library',<FileImageOutlined/>,'cms.manage'),
        ]),
    ]),
    menu('Administration', <SafetyCertificateOutlined/>, [entry('central.settings.index','Platform Settings',<SettingOutlined/>,'settings.view'),entry('central.notifications.index','Notifications',<NotificationOutlined/>),entry('central.central-admins.index','Admin Users',<UserOutlined/>,'admin.manage'),entry('central.roles.index','Roles and Permissions',<SafetyCertificateOutlined/>,'role.manage'),entry('central.audit-logs.index','Audit Logs',<AuditOutlined/>,'audit.view')]),
];
...
export default function CentralLayout({ title, subtitle, breadcrumbs = [], children }) {
    ...
    const visibleNavigation = useMemo(() => {
        const allowed = (item) => !item.permission || bypass || permissions.includes(item.permission);
        const filterItems = (items) => items.map((item) => {
            ...
            return allowed(item) && route().has(item.routeName) ? item : null;
        }).filter(Boolean);
    ...
    const allNav = useMemo(() => {
        ...
        const flattenItems = (navItems, parents = []) => {
            navItems.forEach((item) => {
                ...
                items.push({ ...item, parents, key: navKey(parents, item.label), url: route(item.routeName, item.params) });
            });
        };
    ...
    const activeOpenKeys = active?.parents?.map((label, index) => navKey(active.parents.slice(0, index), label)) || [];
    ...
    const menuItems = useMemo(() => {
        const toMenuItems = (items, parents = []) => items.map((item) => {
            const key = navKey(parents, item.label);

            if (item.children) {
                return { key, label: item.label, icon: item.icon, children: toMenuItems(item.children, [...parents, item.label]) };
            }

            return { key, label: item.label, icon: item.icon, onClick:()=>{setMobileOpen(false);router.visit(route(item.routeName,item.params));} };
        });
    ...
    const renderMenu = (isCollapsed) => <><div className="central-sider__brand"><ApplicationLogo dark className="central-sider__logo" alt="KiteLedger"/></div><div className="central-sider__menu"><Menu theme="dark" mode="inline" inlineCollapsed={isCollapsed} selectedKeys={[active?.key]} defaultOpenKeys={isCollapsed ? [] : activeOpenKeys} items={menuItems}/></div></>;
