...
import {
    ...
    CheckCircleOutlined, CloudServerOutlined, CodeSandboxOutlined, CreditCardOutlined, DashboardOutlined,
...
export default function CentralLayout({ title, subtitle, breadcrumbs = [], children }) {
    ...
    const renderMenu = (isCollapsed) => <><div className="central-sider__brand"><ApplicationLogo dark className="central-sider__logo" alt="KiteLedger"/>{!isCollapsed&&<span className="central-sider__brand-copy"><strong>KiteLedger</strong><span>SUPER ADMIN</span></span>}</div><div className="central-sider__menu"><Menu theme="dark" mode="inline" inlineCollapsed={isCollapsed} selectedKeys={[active?.key]} items={menuItems}/></div><div className="central-sider__footer"><CheckCircleOutlined style={{color:'#34d399',marginRight:9}}/>{!isCollapsed&&'Platform operational'}</div></>;
