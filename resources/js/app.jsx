...
const syncFavicon = (settings) => {
    const faviconUrl = resolveMediaUrl(
        settings?.favicon_url || settings?.favicon,
    );
    const selectors = [
    ...
    const upsertLink = (key, rel, withType = true) => {
        let link = document.querySelector(
            `link[data-kiteledger-favicon="${key}"]`,
        );
...
function RootApp({ App, props }) {
    ...
    const isCentralSurface =
        props?.initialPage?.component?.startsWith("Central/");
    ...
    useEffect(() => {
        ...
        if (!isCentralSurface) {
            fetchBrandSettings()
                ...
                .catch((error) => {
                    console.error(
                        "Failed to load company brand settings:",
                        error,
                    );
                });
        }
...
createInertiaApp({
    ...
    resolve: (name) =>
        resolvePageComponent(
            `./Pages/${name}.jsx`,
            import.meta.glob(["./Pages/**/*.jsx", '!./Pages/App/AI/**']),
        ),
