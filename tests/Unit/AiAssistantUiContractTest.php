...
class AiAssistantUiContractTest extends TestCase
{
    public function test_assistant_uses_production_components_and_safe_runtime_contracts(): void
    {
        ...
        $this->assertStringContainsString('<PreparingAi', $source);
        $this->assertStringContainsString('Preparing AI', $source);
        $this->assertStringContainsString('window.sessionStorage', $source);
        $this->assertStringContainsString('AI_HEALTH_CACHE_TTL_MS', $source);
        $this->assertStringContainsString('aria-busy={healthLoading || sending}', $source);
        ...
        $this->assertStringContainsString('branchContext?.current_fiscal_year_id', $source);
