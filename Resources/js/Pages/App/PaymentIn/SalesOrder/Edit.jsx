    ...
    <Row style={{ marginTop: 0 }}>
      <Col xs={24}>
        <div className="sales-order-total-card">
          <div className="sales-order-total-row"><span>Sub Total</span><strong>{moneyWithSymbol(summary.subTotal, symbol)}</strong></div>
          <div className="sales-order-total-row"><span>Discount</span><strong>{summary.discount > 0 ? moneyWithSymbol(summary.discount, symbol) : '-'}</strong></div>
          <div className="sales-order-total-row"><span>Non-Taxable Total</span><strong>{summary.nonTaxableTotal > 0 ? moneyWithSymbol(summary.nonTaxableTotal, symbol) : '-'}</strong></div>
          <div className="sales-order-total-row"><span>Taxable Total</span><strong>{summary.taxableTotal > 0 ? moneyWithSymbol(summary.taxableTotal, symbol) : '-'}</strong></div>
          <div className="sales-order-total-row"><span>VAT</span><strong>{summary.vat > 0 ? moneyWithSymbol(summary.vat, symbol) : '-'}</strong></div>
          <div className="sales-order-total-row sales-order-total-grand"><span>Grand Total</span><strong>{moneyWithSymbol(summary.grandTotal, symbol)}</strong></div>
        </div>
      </Col>
    </Row>
  );
...
const visitSalesOrderShow = (id) => {
  if (!id) return;
  if (typeof route === 'function') { router.visit(route('payment-in.sales-orders.show', id)); return; }
  router.visit(`/payment-in/sales-orders/${id}`);
};

export default function SalesOrderEdit({ id, ...props }) {
  const fields = useMemo(() => [
    ...
    { name: 'sales_order_no', label: 'SO No', type: 'text', col: 8, placeholder: 'Auto-generated', disabled: true },
    { name: 'sales_order_date', label: 'Date', type: 'datePicker', required: true, col: 8, format: 'DD-MM-YYYY' },
    ...
    { name: '_sales_order_totals', label: '', type: 'custom', col: 12, render: ({ values }) => <SalesOrderTotals values={values} /> },
  ], []);

  const validationSchema = useMemo(() => Yup.object().shape({
    contact_id: Yup.mixed().test('req', 'Customer is required', (v) => !!asId(v)).required('Customer is required'),
    sales_order_date: Yup.mixed().required('Date is required'),
  ...
  const transformPayload = (values = {}) => {
    ...
    return {
      sales_order_no: nullIfEmpty(values.sales_order_no),
  ...
  return (
    <AuthenticatedLayout auth={props.auth} user={props.auth?.user}>
      ...
      <style>{`
        .sales-order-form-drawer .ant-drawer-content { background: #ffffff; }
        .sales-order-form-drawer .ant-drawer-header { min-height: 38px; padding: 0 14px; border-bottom: 1px solid #d9dee7; background: #ffffff; }
        .sales-order-form-drawer .ant-drawer-title { font-size: 14px; font-weight: 700; color: #1f2937; }
        .sales-order-form-drawer .ant-drawer-body { padding: 12px 14px 24px; background: #ffffff; }
        .sales-order-form-drawer .ant-form { max-width: 100%; margin: 0; padding: 0; }
        .sales-order-form-drawer .ant-form-item { margin-bottom: 12px; }
        .sales-order-form-drawer .ant-form-item-label { padding-bottom: 3px; }
        .sales-order-form-drawer .ant-form-item-label > label { height: auto; font-size: 12px; font-weight: 600; color: #1f2937; }
        .sales-order-form-drawer .ant-input, .sales-order-form-drawer .ant-input-number, .sales-order-form-drawer .ant-picker, .sales-order-form-drawer .ant-select-selector { min-height: 31px; border-radius: 0 !important; border-color: #cbd5e1 !important; background: #ffffff; box-shadow: none !important; }
        .sales-order-form-drawer .ant-input-number { width: 100%; }
        .sales-order-form-drawer textarea.ant-input { min-height: 76px; resize: vertical; }
        .sales-order-form-drawer .ant-input[disabled], .sales-order-form-drawer .ant-input-number-disabled, .sales-order-form-drawer .ant-select-disabled .ant-select-selector { color: #111827; background: #eef2f7 !important; }
        .sales-order-form-drawer .ant-btn-primary { min-width: 88px; height: 31px; border-radius: 0; background: #16a34a; border-color: #16a34a; font-weight: 700; }
        .sales-order-form-drawer .ant-btn-primary:hover { background: #15803d !important; border-color: #15803d !important; }
        .sales-order-form-drawer .ant-btn { border-radius: 0; }
        .sales-order-form-drawer .ant-table { border: 1px solid #cbd5e1; border-radius: 0; }
        .sales-order-form-drawer .ant-table-thead > tr > th { background: #4b5563 !important; color: #ffffff !important; font-size: 12px; font-weight: 700; border-bottom: 0; }
        .sales-order-form-drawer .ant-table-tbody > tr > td { padding: 7px 10px; vertical-align: top; }
        .sales-order-form-drawer .ant-table-tbody .ant-form-item { margin-bottom: 0; }
        .sales-order-total-card { border: 1px solid #d8dee8; background: #eef2f8; }
        .sales-order-total-row { display: flex; justify-content: space-between; gap: 16px; padding: 9px 12px; font-size: 13px; color: #111827; }
        .sales-order-total-row strong { font-weight: 600; }
        .sales-order-total-grand { margin-top: 8px; padding-top: 14px; padding-bottom: 14px; border-top: 1px solid #d8dee8; font-size: 15px; font-weight: 700; }
        .sales-order-total-grand strong { font-size: 18px; }
      `}</style>
      <ReusableCrud
        className="sales-order-crud"
        drawerClassName="sales-order-form-drawer"
        title="Sales Orders"
        ...
        look_up_var={id}
        drawerWidth="100vw"
        onEditSuccess={(record) => { visitSalesOrderShow(record?.id || id); }}
      />
    </AuthenticatedLayout>
  );
}
