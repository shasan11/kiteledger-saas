...
const calcLine = (l) => {
    const gross = toNumber(l.qty) * toNumber(l.unit_price);
    return Math.max(gross - gross * (toNumber(l.discount_percent) / 100) + toNumber(l.tax_amount), 0);
};
...
export default function SalesOrderAdd(props) {
    ...
    const crudInitialValues = useMemo(() => ({
        sales_order_no: '', sales_order_date: dayjs(), due_date: null, contact_id: null, warehouse_id: null, currency_id: null, exchange_rate: 1, reference: '', notes: '', items: [{ ...emptyLine }], deleted_item_ids: [],
    }), []);
    ...
    return (
        <AuthenticatedLayout user={props.auth?.user}>
            ...
            <ReusableCrud
                ...
                onAddSuccess={(record) => router.visit(route('payment-in.sales-orders.show', record.id))}
            />
        </AuthenticatedLayout>
    );
}
