...
import { Col, InputNumber, Row, Select, Space, Tag, Typography } from 'antd';
...
const productQuickAdd = {
  ...
  initialValues: { type_of_product: 'goods', name: '', code: '', product_category_id: null, tax_class_id: null, product_unit_id: null, purchase_price: 0, selling_price: 0, allow_sale: true, track_inventory: true, allow_purchase: true, product_type: 'simple', valuation_method: 'standard', reorder_level: 0, description: '' },
  ...
  fields: [{
    type: 'group', label: '', col: 24, accordion: false, children: [
      ...
      { name: 'allow_sale', label: 'Available For Sale', type: 'switch', col: 12 },
    ],
  }],
  transformPayload: (v) => {
    ...
    <Row style={{ marginTop: 0 }}>
      <Col xs={24}>
        <div className="bill-total-card">
          <div className="bill-total-row"><span>Sub Total</span><strong>{moneyWithSymbol(summary.subTotal, symbol)}</strong></div>
          <div className="bill-total-row"><span>Discount</span><strong>{summary.discount > 0 ? moneyWithSymbol(summary.discount, symbol) : '-'}</strong></div>
          <div className="bill-total-row"><span>Non-Taxable Total</span><strong>{summary.nonTaxableTotal > 0 ? moneyWithSymbol(summary.nonTaxableTotal, symbol) : '-'}</strong></div>
          <div className="bill-total-row"><span>Taxable Total</span><strong>{summary.taxableTotal > 0 ? moneyWithSymbol(summary.taxableTotal, symbol) : '-'}</strong></div>
          <div className="bill-total-row"><span>VAT</span><strong>{summary.vat > 0 ? moneyWithSymbol(summary.vat, symbol) : '-'}</strong></div>
          <div className="bill-total-row bill-total-grand"><span>Grand Total</span><strong>{moneyWithSymbol(summary.grandTotal, symbol)}</strong></div>
        </div>
      </Col>
    </Row>
  );
...
const displayDate = (v) => { const d = formatDate(v); return d ? dayjs(d).format('DD-MM-YYYY') : '-'; };
const getContactName = (r) => r?.contact?.name || r?.contact_name || r?.contact_id_detail?.label || '-';

const visitBillShow = (id) => {
  if (!id) return;
  if (typeof route === 'function') { router.visit(route('payment-in.bills.show', id)); return; }
  router.visit(`/payment-in/bills/${id}`);
};
...
export default function BillAdd(props) {
  ...
  const columns = useMemo(() => [
    ...
    { title: 'Customer', dataIndex: 'contact', key: 'contact', render: (_, r) => getContactName(r) },
    { title: 'Date', dataIndex: 'invoice_date', key: 'invoice_date', sorter: true, width: 130, render: displayDate },
    { title: 'Due Date', dataIndex: 'due_date', key: 'due_date', sorter: true, width: 130, render: displayDate },
    ...
    { title: 'Amount', dataIndex: 'total', key: 'total', sorter: true, align: 'right', width: 150, render: (v, r) => <Text strong>{moneyWithSymbol(v, getCurrencySymbol(r))}</Text> },
  ], []);

  const fields = useMemo(() => [
    {
      name: 'contact_id', label: 'Customer Name', type: 'fkSelect', required: true, col: 16,
      placeholder: 'Select Customer', fkUrl: api('/api/contacts/'), fkSearchParam: 'search', fkPageSize: 20, fkValueKey: 'id', fkLabelKey: 'name',
      quickAdd: {
        title: 'Customer', buttonLabel: 'Add New Customer', apiUrl: api('/api/contacts/'),
        initialValues: { contact_type: 'customer', type: 'customer', name: '', code: '', phone: '', email: '', address: '', active: true },
        ...
        fields: [
          ...
          { name: 'phone', label: 'Phone', type: 'phone', col: 12, defaultCountryCode: '+977', placeholder: '9800000000' },
          { name: 'email', label: 'Email', type: 'text', col: 12 },
          { name: 'address', label: 'Address', type: 'textarea', col: 24, rows: 2 },
        ],
        transformPayload: (v) => ({ ...v, contact_type: v.contact_type || 'customer', type: v.type || 'customer', code: v.code?.trim() || null, active: true }),
      },
    },
    { name: 'invoice_no', label: 'Bill No', type: 'text', col: 8, placeholder: 'DRAFT', disabled: true },
      ...
      columns: [
        {
          ...
          onSelectRecord: (r, row) => {
            const unitPrice = toNumber(r?.selling_price ?? r?.sale_price ?? r?.price ?? row?.unit_price);
  ...
  const transformPayload = (values = {}) => {
    ...
    return {
      invoice_no: values.invoice_no && values.invoice_no !== 'DRAFT' ? values.invoice_no : null,
  ...
  return (
    <AuthenticatedLayout auth={props.auth} user={props.auth?.user}>
      <Head title="Bills" />
      <style>{`
        .bill-form-drawer .ant-drawer-content { background: #ffffff; }
        .bill-form-drawer .ant-drawer-header { min-height: 38px; padding: 0 14px; border-bottom: 1px solid #d9dee7; background: #ffffff; }
        .bill-form-drawer .ant-drawer-title { font-size: 14px; font-weight: 700; color: #1f2937; }
        .bill-form-drawer .ant-drawer-body { padding: 12px 14px 24px; background: #ffffff; }
        .bill-form-drawer .ant-form { max-width: 100%; margin: 0; padding: 0; }
        .bill-form-drawer .ant-form-item { margin-bottom: 12px; }
        .bill-form-drawer .ant-form-item-label { padding-bottom: 3px; }
        .bill-form-drawer .ant-form-item-label > label { height: auto; font-size: 12px; font-weight: 600; color: #1f2937; }
        .bill-form-drawer .ant-input, .bill-form-drawer .ant-input-number, .bill-form-drawer .ant-picker, .bill-form-drawer .ant-select-selector { min-height: 31px; border-radius: 0 !important; border-color: #cbd5e1 !important; background: #ffffff; box-shadow: none !important; }
        .bill-form-drawer .ant-input-number { width: 100%; }
        .bill-form-drawer textarea.ant-input { min-height: 76px; resize: vertical; }
        .bill-form-drawer .ant-input[disabled], .bill-form-drawer .ant-input-number-disabled, .bill-form-drawer .ant-select-disabled .ant-select-selector { color: #111827; background: #eef2f7 !important; }
        .bill-form-drawer .ant-btn-primary { min-width: 88px; height: 31px; border-radius: 0; background: #16a34a; border-color: #16a34a; font-weight: 700; }
        .bill-form-drawer .ant-btn-primary:hover { background: #15803d !important; border-color: #15803d !important; }
        .bill-form-drawer .ant-btn { border-radius: 0; }
        .bill-form-drawer .ant-table { border: 1px solid #cbd5e1; border-radius: 0; }
        .bill-form-drawer .ant-table-thead > tr > th { background: #4b5563 !important; color: #ffffff !important; font-size: 12px; font-weight: 700; border-bottom: 0; }
        .bill-form-drawer .ant-table-tbody > tr > td { padding: 7px 10px; vertical-align: top; }
        .bill-form-drawer .ant-table-tbody .ant-form-item { margin-bottom: 0; }
        .bill-total-card { border: 1px solid #d8dee8; background: #eef2f8; }
        .bill-total-row { display: flex; justify-content: space-between; gap: 16px; padding: 9px 12px; font-size: 13px; color: #111827; }
        .bill-total-row strong { font-weight: 600; }
        .bill-total-grand { margin-top: 8px; padding-top: 14px; padding-bottom: 14px; border-top: 1px solid #d8dee8; font-size: 15px; font-weight: 700; }
      ...
      <ReusableCrud
        ...
        hasActions
        hasActionColumns
        onAddSuccess={(record) => { if (record?.id) visitBillShow(record.id); }}
        onEditSuccess={(record) => { if (record?.id) visitBillShow(record.id); }}
        activeTableRowFunction={(record) => ({
          onClick: (event) => { const ignored = event.target.closest('button,a,input,textarea,.ant-checkbox-wrapper,.ant-dropdown-trigger,.ant-select,.ant-picker,.ant-btn,.ant-dropdown'); if (ignored) return; visitBillShow(record.id); },
