    ...
    <Row style={{ marginTop: 0 }}>
      <Col xs={24}>
        <div className="credit-note-total-card">
          <div className="credit-note-total-row"><span>Sub Total</span><strong>{moneyWithSymbol(summary.subTotal, symbol)}</strong></div>
          <div className="credit-note-total-row"><span>Discount</span><strong>{summary.discount > 0 ? moneyWithSymbol(summary.discount, symbol) : '-'}</strong></div>
          <div className="credit-note-total-row"><span>Non-Taxable Total</span><strong>{summary.nonTaxableTotal > 0 ? moneyWithSymbol(summary.nonTaxableTotal, symbol) : '-'}</strong></div>
          <div className="credit-note-total-row"><span>Taxable Total</span><strong>{summary.taxableTotal > 0 ? moneyWithSymbol(summary.taxableTotal, symbol) : '-'}</strong></div>
          <div className="credit-note-total-row"><span>VAT</span><strong>{summary.vat > 0 ? moneyWithSymbol(summary.vat, symbol) : '-'}</strong></div>
          <div className="credit-note-total-row credit-note-total-grand"><span>Credit Total</span><strong>{moneyWithSymbol(summary.grandTotal, symbol)}</strong></div>
        </div>
      </Col>
    </Row>
  );
...
const visitCreditNoteShow = (id) => {
  if (!id) return;
  if (typeof route === 'function') { router.visit(route('payment-in.credit-notes.show', id)); return; }
  router.visit(`/payment-in/credit-notes/${id}`);
};

export default function CreditNoteEdit({ id, ...props }) {
  const fields = useMemo(() => [
    ...
    { name: '_credit_note_totals', label: '', type: 'custom', col: 12, render: ({ values }) => <CreditNoteTotals values={values} /> },
  ], []);
  ...
  return (
    <AuthenticatedLayout auth={props.auth} user={props.auth?.user}>
      ...
      <style>{`
        .credit-note-form-drawer .ant-drawer-content { background: #ffffff; }
        .credit-note-form-drawer .ant-drawer-header { min-height: 38px; padding: 0 14px; border-bottom: 1px solid #d9dee7; background: #ffffff; }
        .credit-note-form-drawer .ant-drawer-title { font-size: 14px; font-weight: 700; color: #1f2937; }
        .credit-note-form-drawer .ant-drawer-body { padding: 12px 14px 24px; background: #ffffff; }
        .credit-note-form-drawer .ant-form { max-width: 100%; margin: 0; padding: 0; }
        .credit-note-form-drawer .ant-form-item { margin-bottom: 12px; }
        .credit-note-form-drawer .ant-form-item-label { padding-bottom: 3px; }
        .credit-note-form-drawer .ant-form-item-label > label { height: auto; font-size: 12px; font-weight: 600; color: #1f2937; }
        .credit-note-form-drawer .ant-input, .credit-note-form-drawer .ant-input-number, .credit-note-form-drawer .ant-picker, .credit-note-form-drawer .ant-select-selector { min-height: 31px; border-radius: 0 !important; border-color: #cbd5e1 !important; background: #ffffff; box-shadow: none !important; }
        .credit-note-form-drawer .ant-input-number { width: 100%; }
        .credit-note-form-drawer textarea.ant-input { min-height: 76px; resize: vertical; }
        .credit-note-form-drawer .ant-input[disabled], .credit-note-form-drawer .ant-input-number-disabled, .credit-note-form-drawer .ant-select-disabled .ant-select-selector { color: #111827; background: #eef2f7 !important; }
        .credit-note-form-drawer .ant-btn-primary { min-width: 88px; height: 31px; border-radius: 0; background: #16a34a; border-color: #16a34a; font-weight: 700; }
        .credit-note-form-drawer .ant-btn-primary:hover { background: #15803d !important; border-color: #15803d !important; }
        .credit-note-form-drawer .ant-btn { border-radius: 0; }
        .credit-note-form-drawer .ant-table { border: 1px solid #cbd5e1; border-radius: 0; }
        .credit-note-form-drawer .ant-table-thead > tr > th { background: #4b5563 !important; color: #ffffff !important; font-size: 12px; font-weight: 700; border-bottom: 0; }
        .credit-note-form-drawer .ant-table-tbody > tr > td { padding: 7px 10px; vertical-align: top; }
        .credit-note-form-drawer .ant-table-tbody .ant-form-item { margin-bottom: 0; }
        .credit-note-total-card { border: 1px solid #d8dee8; background: #eef2f8; }
        .credit-note-total-row { display: flex; justify-content: space-between; gap: 16px; padding: 9px 12px; font-size: 13px; color: #111827; }
        .credit-note-total-row strong { font-weight: 600; }
        .credit-note-total-grand { margin-top: 8px; padding-top: 14px; padding-bottom: 14px; border-top: 1px solid #d8dee8; font-size: 15px; font-weight: 700; }
        .credit-note-total-grand strong { font-size: 18px; }
      `}</style>
      <ReusableCrud
        className="credit-note-crud"
        ...
        look_up_var={id}
        drawerWidth="100vw"
        onEditSuccess={(record) => { visitCreditNoteShow(record?.id || id); }}
      />
    </AuthenticatedLayout>
  );
}
