...
const PurchaseOrderTotals = ({ values = {} }) => {
  const summary = calculateSummary(values.items || []);
  const symbol = getCurrencySymbol(values);
  return (
    <Row style={{ marginTop: 0 }}>
      <Col xs={24}>
        <div className="purchase-order-total-card">
          <div className="purchase-order-total-row"><span>Sub Total</span><strong>{moneyWithSymbol(summary.subTotal, symbol)}</strong></div>
          <div className="purchase-order-total-row"><span>Discount</span><strong>{summary.discount > 0 ? moneyWithSymbol(summary.discount, symbol) : '-'}</strong></div>
          <div className="purchase-order-total-row"><span>Non-Taxable Total</span><strong>{summary.nonTaxableTotal > 0 ? moneyWithSymbol(summary.nonTaxableTotal, symbol) : '-'}</strong></div>
          <div className="purchase-order-total-row"><span>Taxable Total</span><strong>{summary.taxableTotal > 0 ? moneyWithSymbol(summary.taxableTotal, symbol) : '-'}</strong></div>
          <div className="purchase-order-total-row"><span>VAT</span><strong>{summary.vat > 0 ? moneyWithSymbol(summary.vat, symbol) : '-'}</strong></div>
          <div className="purchase-order-total-row purchase-order-total-grand"><span>Grand Total</span><strong>{moneyWithSymbol(summary.grandTotal, symbol)}</strong></div>
        </div>
      </Col>
    </Row>
  );
...
const visitPurchaseOrderShow = (id) => {
  if (!id) return;
  if (typeof route === 'function') { router.visit(route('payment-out.purchase-orders.show', id)); return; }
  router.visit(`/payment-out/purchase-orders/${id}`);
};

export default function PurchaseOrderAdd(props) {
  ...
  const fields = useMemo(() => [
    ...
    { name: 'purchase_order_date', label: 'PO Date', type: 'datePicker', required: true, col: 8, format: 'DD-MM-YYYY' },
    ...
    { name: '_purchase_order_totals', label: '', type: 'custom', col: 12, render: ({ values }) => <PurchaseOrderTotals values={values} /> },
  ], [baseCurrency]);
  ...
  const crudInitialValues = useMemo(() => ({
    purchase_order_no: null, purchase_order_date: dayjs(), due_date: null,
  ...
  return (
    <AuthenticatedLayout auth={props.auth} user={props.auth?.user}>
      ...
      <style>{`
        .purchase-order-form-drawer .ant-drawer-content { background: #ffffff; }
        .purchase-order-form-drawer .ant-drawer-header { min-height: 38px; padding: 0 14px; border-bottom: 1px solid #d9dee7; background: #ffffff; }
        .purchase-order-form-drawer .ant-drawer-title { font-size: 14px; font-weight: 700; color: #1f2937; }
        .purchase-order-form-drawer .ant-drawer-body { padding: 12px 14px 24px; background: #ffffff; }
        .purchase-order-form-drawer .ant-form { max-width: 100%; margin: 0; padding: 0; }
        .purchase-order-form-drawer .ant-form-item { margin-bottom: 12px; }
        .purchase-order-form-drawer .ant-form-item-label { padding-bottom: 3px; }
        .purchase-order-form-drawer .ant-form-item-label > label { height: auto; font-size: 12px; font-weight: 600; color: #1f2937; }
        .purchase-order-form-drawer .ant-input, .purchase-order-form-drawer .ant-input-number, .purchase-order-form-drawer .ant-picker, .purchase-order-form-drawer .ant-select-selector { min-height: 31px; border-radius: 0 !important; border-color: #cbd5e1 !important; background: #ffffff; box-shadow: none !important; }
        .purchase-order-form-drawer .ant-input-number { width: 100%; }
        .purchase-order-form-drawer textarea.ant-input { min-height: 76px; resize: vertical; }
        .purchase-order-form-drawer .ant-input[disabled], .purchase-order-form-drawer .ant-input-number-disabled, .purchase-order-form-drawer .ant-select-disabled .ant-select-selector { color: #111827; background: #eef2f7 !important; }
        .purchase-order-form-drawer .ant-btn-primary { min-width: 88px; height: 31px; border-radius: 0; background: #2563eb; border-color: #2563eb; font-weight: 700; }
        .purchase-order-form-drawer .ant-btn-primary:hover { background: #1d4ed8 !important; border-color: #1d4ed8 !important; }
        .purchase-order-form-drawer .ant-btn { border-radius: 0; }
        .purchase-order-form-drawer .ant-table { border: 1px solid #cbd5e1; border-radius: 0; }
        .purchase-order-form-drawer .ant-table-thead > tr > th { background: #4b5563 !important; color: #ffffff !important; font-size: 12px; font-weight: 700; border-bottom: 0; }
        .purchase-order-form-drawer .ant-table-tbody > tr > td { padding: 7px 10px; vertical-align: top; }
        .purchase-order-form-drawer .ant-table-tbody .ant-form-item { margin-bottom: 0; }
        .purchase-order-total-card { border: 1px solid #d8dee8; background: #eef2f8; }
        .purchase-order-total-row { display: flex; justify-content: space-between; gap: 16px; padding: 9px 12px; font-size: 13px; color: #111827; }
        .purchase-order-total-row strong { font-weight: 600; }
        .purchase-order-total-grand { margin-top: 8px; padding-top: 14px; padding-bottom: 14px; border-top: 1px solid #d8dee8; font-size: 15px; font-weight: 700; }
        .purchase-order-total-grand strong { font-size: 18px; }
      `}</style>
      <ReusableCrud
        key={baseCurrency?.id || 'purchase-order-add'}
        className="purchase-order-crud"
        drawerClassName="purchase-order-form-drawer"
        ...
        ui_type="add form"
        drawerWidth="100vw"
        onAddSuccess={(record) => { if (record?.id) visitPurchaseOrderShow(record.id); }}
      />
    </AuthenticatedLayout>
  );
}
