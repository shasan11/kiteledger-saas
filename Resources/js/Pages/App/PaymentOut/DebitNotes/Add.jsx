...
const DebitNoteTotals = ({ values = {} }) => {
  const summary = calculateSummary(values.items || []);
  return (
    <Row style={{ marginTop: 0 }}>
      <Col xs={24}>
        <div className="debit-note-total-card">
          <div className="debit-note-total-row"><span>Sub Total</span><strong>{moneyWithSymbol(summary.subTotal)}</strong></div>
          <div className="debit-note-total-row"><span>Discount</span><strong>{summary.discount > 0 ? moneyWithSymbol(summary.discount) : '-'}</strong></div>
          <div className="debit-note-total-row"><span>Non-Taxable Total</span><strong>{summary.nonTaxableTotal > 0 ? moneyWithSymbol(summary.nonTaxableTotal) : '-'}</strong></div>
          <div className="debit-note-total-row"><span>Taxable Total</span><strong>{summary.taxableTotal > 0 ? moneyWithSymbol(summary.taxableTotal) : '-'}</strong></div>
          <div className="debit-note-total-row"><span>VAT</span><strong>{summary.vat > 0 ? moneyWithSymbol(summary.vat) : '-'}</strong></div>
          <div className="debit-note-total-row debit-note-total-grand"><span>Debit Total</span><strong>{moneyWithSymbol(summary.grandTotal)}</strong></div>
        </div>
      </Col>
    </Row>
  );
...
const visitDebitNoteShow = (id) => {
  if (!id) return;
  if (typeof route === 'function') { router.visit(route('payment-out.debit-notes.show', id)); return; }
  router.visit(`/payment-out/debit-notes/${id}`);
};

export default function DebitNoteAdd(props) {
  const fields = useMemo(() => [
    ...
    },
    {
      ...
      columns: [
        ...
        { key: 'unit_price', name: 'unit_price', label: 'Rate', type: 'number', width: '15%', min: 0 },
        ...
        { key: 'line_total', name: 'line_total', label: 'Amount', type: 'number', width: '13%', min: 0, disabled: true, formula: (row) => calculateLine(row).line_total },
      ],
      collapsedFields: [
    ...
    { name: '_debit_note_totals', label: '', type: 'custom', col: 12, render: ({ values }) => <DebitNoteTotals values={values} /> },
  ], []);
  ...
  const transformPayload = (values = {}) => {
    ...
    return {
      debit_note_no: nullIfEmpty(values.debit_note_no),
  ...
  return (
    <AuthenticatedLayout auth={props.auth} user={props.auth?.user}>
      ...
      <style>{`
        .debit-note-form-drawer .ant-drawer-content { background: #ffffff; }
        .debit-note-form-drawer .ant-drawer-header { min-height: 38px; padding: 0 14px; border-bottom: 1px solid #d9dee7; background: #ffffff; }
        .debit-note-form-drawer .ant-drawer-title { font-size: 14px; font-weight: 700; color: #1f2937; }
        .debit-note-form-drawer .ant-drawer-body { padding: 12px 14px 24px; background: #ffffff; }
        .debit-note-form-drawer .ant-form { max-width: 100%; margin: 0; padding: 0; }
        .debit-note-form-drawer .ant-form-item { margin-bottom: 12px; }
        .debit-note-form-drawer .ant-form-item-label { padding-bottom: 3px; }
        .debit-note-form-drawer .ant-form-item-label > label { height: auto; font-size: 12px; font-weight: 600; color: #1f2937; }
        .debit-note-form-drawer .ant-input, .debit-note-form-drawer .ant-input-number, .debit-note-form-drawer .ant-picker, .debit-note-form-drawer .ant-select-selector { min-height: 31px; border-radius: 0 !important; border-color: #cbd5e1 !important; background: #ffffff; box-shadow: none !important; }
        .debit-note-form-drawer .ant-input-number { width: 100%; }
        .debit-note-form-drawer textarea.ant-input { min-height: 76px; resize: vertical; }
        .debit-note-form-drawer .ant-input[disabled], .debit-note-form-drawer .ant-input-number-disabled, .debit-note-form-drawer .ant-select-disabled .ant-select-selector { color: #111827; background: #eef2f7 !important; }
        .debit-note-form-drawer .ant-btn-primary { min-width: 88px; height: 31px; border-radius: 0; background: #2563eb; border-color: #2563eb; font-weight: 700; }
        .debit-note-form-drawer .ant-btn-primary:hover { background: #1d4ed8 !important; border-color: #1d4ed8 !important; }
        .debit-note-form-drawer .ant-btn { border-radius: 0; }
        .debit-note-form-drawer .ant-table { border: 1px solid #cbd5e1; border-radius: 0; }
        .debit-note-form-drawer .ant-table-thead > tr > th { background: #4b5563 !important; color: #ffffff !important; font-size: 12px; font-weight: 700; border-bottom: 0; }
        .debit-note-form-drawer .ant-table-tbody > tr > td { padding: 7px 10px; vertical-align: top; }
        .debit-note-form-drawer .ant-table-tbody .ant-form-item { margin-bottom: 0; }
        .debit-note-total-card { border: 1px solid #d8dee8; background: #eef2f8; }
        .debit-note-total-row { display: flex; justify-content: space-between; gap: 16px; padding: 9px 12px; font-size: 13px; color: #111827; }
        .debit-note-total-row strong { font-weight: 600; }
        .debit-note-total-grand { margin-top: 8px; padding-top: 14px; padding-bottom: 14px; border-top: 1px solid #d8dee8; font-size: 15px; font-weight: 700; }
        .debit-note-total-grand strong { font-size: 18px; }
      `}</style>
      <ReusableCrud
        className="debit-note-crud"
        drawerClassName="debit-note-form-drawer"
        title="Debit Notes"
        addTitle="New Debit Note"
        ...
        ui_type="add form"
        drawerWidth="100vw"
        onAddSuccess={(record) => { if (record?.id) visitDebitNoteShow(record.id); }}
      />
    </AuthenticatedLayout>
  );
}
