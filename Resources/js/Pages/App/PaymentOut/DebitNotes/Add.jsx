...
const calcLine = (l) => Math.max(toNumber(l.qty) * toNumber(l.unit_price) + toNumber(l.tax_amount), 0);
const emptyLine = { id: undefined, product_id: null, description: '', qty: 1, unit_price: 0, tax_rate_id: null, tax_amount: 0, line_total: 0 };
const normalizeLine = (l = {}) => {
    const n = { ...(l.id ? { id: l.id } : {}), product_id: asId(l.product_id ?? l.product), description: nullIfEmpty(l.description), qty: toNumber(l.qty) || 0, unit_price: toNumber(l.unit_price), tax_rate_id: asId(l.tax_rate_id ?? l.taxRate ?? l.tax_rate), tax_amount: toNumber(l.tax_amount) };
...
export default function DebitNoteAdd(props) {
    ...
    const validationSchema = useMemo(() => Yup.object().shape({
        ...
        items: Yup.array().of(Yup.object().shape({ qty: Yup.number().typeError('Number').min(0).required(), unit_price: Yup.number().typeError('Number').min(0).required() })).min(1).required(),
    }), []);

    const crudInitialValues = useMemo(() => ({
        debit_note_no: '', debit_note_date: dayjs(), contact_id: null, warehouse_id: null, notes: '', items: [{ ...emptyLine }], deleted_item_ids: [],
    }), []);

    const transformPayload = (values = {}) => {
        const items = (Array.isArray(values.items) ? values.items : []).map(normalizeLine).filter((l) => toNumber(l.qty) > 0);
        return {
    ...
    return (
        <AuthenticatedLayout user={props.auth?.user}>
            ...
            <ReusableCrud
                ...
                onAddSuccess={(record) => router.visit(route('payment-out.debit-notes.show', record.id))}
            />
        </AuthenticatedLayout>
    );
}
