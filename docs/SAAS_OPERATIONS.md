...
Tenant operational state and subscription billing state are independent. Billing expiry restricts access but never deletes tenant data. Permanent deletion requires a separate delayed request, password and typed confirmation, authorization, a recorded reason, and an immutable audit entry.

Central infrastructure includes tenant-bound sessions, prefixed cache entries, locks, queues, lifecycle records, usage reservations, provisioning attempts, billing records, and deletion requests. ERP records remain exclusively in tenant databases. Tenant models fail closed when tenancy is not initialized in production.
...
KiteLedger does not include an application-level backup module. If recovery copies are required, configure and test them independently through the hosting or database provider.
