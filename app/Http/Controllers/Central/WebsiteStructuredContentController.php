...
class WebsiteStructuredContentController extends Controller
{
    ...
    public function index(Request $request, string $resource)
    {
        ...
        $query = $class::query();
        if ($resource === 'resource-articles') $query->with('category:id,name');
        if (in_array($resource, ['features', 'resource-articles'], true)) $query->with('featuredMedia');
        if ($request->filled('search')) {
            $term = '%'.$request->string('search').'%';
            $query->where(function ($q) use ($term): void {
                foreach (['title', 'name', 'content', 'platform'] as $field) if (in_array($field, $q->getModel()->getConnection()->getSchemaBuilder()->getColumnListing($q->getModel()->getTable()), true)) $q->orWhere($field, 'like', $term);
            });
        }
    ...
    {
        $class = self::MODELS[$resource] ?? abort(404); $model = $class::findOrFail($id); $before = $model->toArray();
    ...
    {
        $class = self::MODELS[$resource] ?? abort(404); $model = $class::findOrFail($id);
        if ($model instanceof ResourceCategory && $model->articles()->exists()) return back()->with('error', 'Move or remove this category’s articles first.');
        $audit->log($request, 'website.'.str_replace('-', '_', $resource).'.deleted', $model, $model->toArray(), []); $model->delete();
    ...
    private function validated(Request $request, string $resource, ?Model $model = null): array
    {
        ...
        if (isset($data['body'])) $data['body'] = SafeHtml::clean($data['body']);
        if (isset($data['content'])) $data['content'] = trim(strip_tags($data['content']));
        if (in_array($resource, ['features','resource-articles'], true) && ($data['status'] ?? '') === 'published' && empty($data['published_at'])) $data['published_at'] = now();
        return $data;
    }
}
