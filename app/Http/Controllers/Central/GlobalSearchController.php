...
class GlobalSearchController extends Controller
{
    public function __invoke(Request $request)
    {
        ...
        ResourceArticle::where(fn($q)=>$q->where('title','like',$like)->orWhere('slug','like',$like))->limit(5)->get()->each(fn($row)=>$results->push(['type'=>'Resource article','title'=>$row->title,'subtitle'=>$row->status,'url'=>route('central.website-structured.index',['resource'=>'resource-articles','search'=>$row->slug])]));
        ContactLocation::where(fn($q)=>$q->where('name','like',$like)->orWhere('address','like',$like))->limit(5)->get()->each(fn($row)=>$results->push(['type'=>'Contact location','title'=>$row->name,'subtitle'=>$row->address,'url'=>route('central.website-structured.index',['resource'=>'contact-locations','search'=>$row->name])]));
        CommunicationCampaign::where('name','like',$like)->limit(5)->get()->each(fn($row)=>$results->push(['type'=>'Campaign','title'=>$row->name,'subtitle'=>$row->channel.' / '.$row->status,'url'=>route('central.communication.index')]));
